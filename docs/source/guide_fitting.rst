...
However, we can use constraints to enforce the following orders: ``R2 ≤ R4 ≤ R3`` and ``Q1 ≤ Q2 ≤ Q3``. To accomplish this, we need to provide the |fit_circuit| function with two additional arguments: ``constraint_expressions`` and ``constraint_variables``.
