...
from pyimpspec.plot.colors import COLOR_ORANGE
from pyimpspec.plot.mpl.markers import MARKER_SQUARE
...
def plot_phase(
    ...
    **kwargs,
    ...
    colors: Optional[Dict[str, str]], optional
        The colors of the markers or lines. Valid keys: 'phase'.

    markers: Optional[Dict[str, str]], optional
        The markers to use when not plotting lines. Valid keys: 'phase'.
    ...
    **kwargs
    ...
    if label is None:
        if hasattr(data, "get_label") and callable(data.get_label):
            ...
            label = ""
    color: str = colors.get("phase", COLOR_ORANGE)
    marker: str = markers.get("phase", MARKER_SQUARE)
    x: Frequencies
    y: Phases
    if (
        ...
        y = -angle(data.get_impedances(num_per_decade=num_per_decade), deg=True)
    else:
        x = data.get_frequencies()
        y = -angle(data.get_impedances(), deg=True)
    if line is True:
        axis.plot(
            x,
            y,
    ...
    if adjust_axes:
        ...
        axis.set_ylabel(r"$-$Phase($Z$) ($^\circ$)")
